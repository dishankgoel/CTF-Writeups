...
class RSA_then_Paillier:
    ...
    def __init__(self, domain: tuple):
        ...
        self.U = inverse((pow(self.G, self.L, self.N * self.N) - 1) // self.N,
                         self.N)
    ...
    def decrypt(self, cip: int) -> int:
        # Check HISTORY
        if hashlib.sha256(cip.to_bytes(256,
                                       'big')).hexdigest() in self.HISTORY:
            return -1

        # Paillier
        cip_rsa = ((pow(cip, self.L, self.N * self.N) - 1) // self.N *
                   self.U) % self.N
...
class Paillier_then_RSA:
    ...
    def decrypt(self, cip: int) -> int:
        ...
        # Paillier
        return ((pow(cip_pai, self.L, self.N * self.N) - 1) // self.N *
                self.U) % self.N
